...
rule pairs:
    ...
    shell:
        """
        python3 {pathScripts}2_cds_extraction/python/cluster_species.py -i {input} -t {params.t} -o {output}
        """
...
rule filter_clusters:
    ...
    shell:
        """
        python3 {pathScripts}2_cds_extraction/python/filter_clusters.py -i {input} -o {output}
        """
...
rule filter_org_data:
    ...
    shell:
        ...
        python3 scripts/2_cds_extraction/python/filter_org_data.py -i {input.clust} -d {input.org_data} -o {output.filtered}
        """