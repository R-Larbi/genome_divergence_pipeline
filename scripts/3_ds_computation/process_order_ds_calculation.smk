...
def get_order_pairs(wildcards):
    cp_output_order = pathResults + "list_pairs_order"
    ORDER_PAIRS = []
    ...
    return expand(pathResults + "seaview_alignment/Per_BUSCO_Alignments/{pair}/full_alignment.dNdS", pair=ORDER_PAIRS)

rule all:
    input:
        pathResults + "seaview_alignment/Alignment_Summaries/order_level_full_alignment.dNdS"

rule separate_by_pair_order:
    """
    ...
    shell:
        ...
        python3 {pathScripts}3_ds_computation/python/separate_by_pair.py -p {wildcards.pair} -b {pathResults}busco_pairs_order -o {output}
        """
...
rule summarize_order:
    input:
        get_order_pairs
    output:
        pathResults + "seaview_alignment/Alignment_Summaries/order_level_" + full_part + "_full_alignment.dNdS"
    shell:
        """
