...
parser.add_argument("-s", "--singles", required=True, type=str, help="Path to output orphans file")
...
def parseTaxa(tax_file: str):
    with open(tax_file, "r") as reader:
        for line in reader.readlines()[1:]:
            ...
            if len(line.strip().split('\t')) < 9 and level == 8:
                taxon = f"Unspecified{args.level}"
            elif len(line.strip().split('\t')) <= level:
                taxon = f"Unspecified{args.level}"
            else:
...
## Function to get the pairs of species and orphans
def generatePairs(tax: dict):
    for taxon in tax.keys():
        if len(tax[taxon]) < 3:
            for i in range(len(tax[taxon])):
                singles.append(tax[taxon][i])
        else:
