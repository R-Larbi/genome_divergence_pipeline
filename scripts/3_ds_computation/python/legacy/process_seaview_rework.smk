...
rule separate_by_pair:
    ...
    shell:
        ...
        while read p; do
            ...
            python3 {pathScripts}3_ds_computation/python/separate_by_pair.py -p "$p" -b {input.busco} -o {pathResults}seaview_alignment/{wildcards.clade}/"$p"/busco_pairs
        done <{input.clust}
        rm {input.clust}
        rm {input.busco}
        touch {output.flag}
...
rule seaview:
    ...
    shell:
        """
        csh scripts/3_ds_computation/csh/Aln_dNdS_run_all.csh {input.pairs} {input.busco} 1 {output}
        """
