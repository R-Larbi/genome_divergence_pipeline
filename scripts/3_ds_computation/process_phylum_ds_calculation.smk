...
def get_phylum_pairs(wildcards):
    cp_output_phylum = pathResults + "list_pairs_phylum"
    PHYLUM_PAIRS = []
    ...
    return expand(pathResults + "seaview_alignment/Per_BUSCO_Alignments/{pair}/full_alignment.dNdS", pair=PHYLUM_PAIRS)

rule all:
    input:
        pathResults + "seaview_alignment/Alignment_Summaries/phylum_level_" + full_part + "_full_alignment.dNdS"

rule separate_by_pair_phylum:
    """
    ...
    shell:
        ...
        python3 {pathScripts}3_ds_computation/python/separate_by_pair.py -p {wildcards.pair} -b {pathResults}busco_pairs_phylum -o {output}
        """
