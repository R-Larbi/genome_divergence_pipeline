...
def get_class_pairs(wildcards):
    cp_output_class = pathResults + "list_pairs_class"
    CLASS_PAIRS = []
    ...
    return expand(pathResults + "seaview_alignment/Per_BUSCO_Alignments/{pair}/full_alignment.dNdS", pair=CLASS_PAIRS)

rule all:
    input:
        pathResults + "seaview_alignment/Alignment_Summaries/class_level_full_alignment.dNdS"

rule separate_by_pair_class:
    """
    ...
    shell:
        ...
        python3 {pathScripts}3_ds_computation/python/separate_by_pair.py -p {wildcards.pair} -b {pathResults}busco_pairs_class -o {output}
        """
...
rule summarize_class:
    input:
        get_class_pairs
    output:
        pathResults + "seaview_alignment/Alignment_Summaries/class_level_" + full_part + "_full_alignment.dNdS"
    shell:
        """
