...
rule all:
    input:
        pathResults + "class_full_representative_species"
...
rule concatenate:
    ...
    shell:
        ...
        for f in {pathResults}seaview_alignment/Alignment_Summaries/class_level_*-*_full_alignment.dNdS;
        do
            tail -n+2 $f
        done >> {output}
        """
...
checkpoint matrix_cluster_class:
    ...
    shell:
        ...
        python3 {pathScripts}3_ds_computation/python/generate_matrix.py -i {input.clust} -d {input.ds} -l "Class" -o {pathResults}Class_Clustering/
        """


"""
Make check for all clusters in class for getting representatives, and then repeat for class
"""
...
rule concatenate_rep_class:
    """
    Concatenate all representatives for class clusters alongside orphans
    """
    input:
        check_class_clusters
    output:
        pathResults + "class_full_representative_species"
    shell:
        """
        cat {pathResults}Class_Clustering/cluster_*_representative_species {pathResults}list_orphans_class > {output}
        """
