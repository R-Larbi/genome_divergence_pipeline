...
def get_genus_pairs(wildcards):
    cp_output_genus = pathResults + "list_pairs_genus"
    GENUS_PAIRS = []
    ...
    return expand(pathResults + "seaview_alignment/Per_BUSCO_Alignments/{pair}/full_alignment.dNdS", pair=GENUS_PAIRS)
...
rule separate_by_pair_genus:
    ...
    shell:
        ...
        python3 {pathScripts}3_ds_computation/python/separate_by_pair.py -p {wildcards.pair} -b {pathResults}busco_pairs_genus -o {output}
        """
...
rule summarize_genus:
    input:
        get_genus_pairs
    output:
        pathResults + "seaview_alignment/Alignment_Summaries/genus_level_" + full_part + "_full_alignment.dNdS"
    shell:
        """
