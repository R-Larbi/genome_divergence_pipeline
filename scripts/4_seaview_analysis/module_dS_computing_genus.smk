...
checkpoint get_genus_list:
    ...
    output:
        ...
        singles = pathResults + "list_singletons_genus"
    shell:
        """
        python3 {pathScripts}4_seaview_analysis/python/generate_pairs.py -i {input.species} -t {input.tax_data} -l "Genus" -o {output.fam} -s {output.singles}
        """
...
rule create_pair_list_genus:
    ...
    output:
        touch(temp("pair_lists_genus_done.flag"))
    shell:
        """
        python3 {pathScripts}4_seaview_analysis/python/create_pairs.py -i {input.pairs} -b {input.busco} -o {pathResults}busco_pairs_genus
        """
...
rule separate_by_pair_genus:
    ...
    shell:
        ...
        python3 {pathScripts}4_seaview_analysis/python/separate_by_pair.py -p {wildcards.pair} -b {pathResults}busco_pairs_genus -o {output}
        """
...
checkpoint matrix_cluster_genus:
    ...
    shell:
        ...
        python3 {pathScripts}4_seaview_alignment/python/generate_matrix.py -i {input} -d {ds} -l "Genus" -o {pathResults}Genus_Clustering/
        """
...
rule concatenate_rep_genus:
    """
    Concatenate all representatives for genus clusters alongside singletons
    """
    input:
        check_genus_clusters
    output:
        pathResults + "Genus_Clustering/full_representative_species"
    shell:
        """
        cat {pathResults}Genus_Clustering/cluster_*_representative_species {pathResults}list_singletons_genus > {output}
        """
